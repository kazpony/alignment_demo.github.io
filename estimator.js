/* estimator.js — 静的計測の補助（キャンバー・統計・CSV） */
(function (global) {
  'use strict';
  const V = {
  ...
  function camberStatic(accelStatic, wheelNormalSensor){
    const up=V.unit(accelStatic), s=V.unit(wheelNormalSensor||[0,0,1]);
  ...
  function derived(toes){const frontTotal=toes.FL+toes.FR,thrust=(toes.RL+toes.RR)/2;
    return{front_total_toe_deg:frontTotal,rear_total_toe_deg:toes.RL+toes.RR,thrust_angle_deg:thrust,front_toe_vs_thrust_deg:frontTotal-2*thrust};}
  function degToMm(deg,trackMm){return Math.tan(deg*DEG)*(trackMm||1500)/2;}
  function toCSV(rows,header){const cols=header||Object.keys(rows[0]||{});const lines=[cols.join(',')];
    for(const r of rows){lines.push(cols.map(c=>{const v=r[c];if(v==null)return'';
      if(typeof v==='string'&&(v.includes(',')||v.includes('"')))return'"'+v.replace(/"/g,'""')+'"';return v;}).join(','));}
    return lines.join('\n');}
  const E={V,RAD,DEG,camberStatic,stats,derived,degToMm,toCSV};
  if(typeof module!=='undefined'&&module.exports) module.exports=E; else global.Estimator=E;
