/* =====================================================================
 * sensors.js  —  センサ抽象レイヤ（磁気対応版）
 *   Generic Sensor API 優先 / DeviceMotion+DeviceOrientation フォールバック。
 *   静的磁気モードのため Magnetometer を購読（生値, uT）。
 *   統一サンプル: { t_ms, accel:[m/s^2], gyro:[rad/s], mag:[uT]|null }
 * ===================================================================== */
(function (global) {
  'use strict';
  const HZ = 50; // 静的計測主体なので50Hzで十分
  function now(){ return (performance&&performance.now)?performance.now():Date.now(); }

  class SensorHub {
    constructor(){
      this.running=false; this.mode=null; this._cb=null; this._sensors=[];
      this._last={accel:null,gyro:[0,0,0],mag:null};
    ...
    async start(cb){
      ...
      if(hasGeneric){
        try{ await this._startGeneric(); this.mode='generic'; this.running=true; this._rate(); return this.mode; }
        catch(e){ console.warn('Generic失敗→DeviceMotion',e); this._stopGeneric(); }
      }
      this._startDeviceMotion(); this.mode='devicemotion'; this.running=true; this._rate();
      return this.mode;
    }
    async _startGeneric(){
      ...
      if('Magnetometer' in global){
        try{
          const mag=new global.Magnetometer(opts);
    ...
    _startDeviceMotion(){
      this._dm=(ev)=>{
        const ig=ev.accelerationIncludingGravity, rr=ev.rotationRate;
      ...
      global.addEventListener('devicemotion',this._dm,true);
      // 磁気は DeviceMotion では取れない → webkitCompassHeading等は方位のみで生ベクトル不可。
      // Generic不可の端末では磁気モードは使えない旨をhasMag=falseで通知。
      this.hasMag=false;
    }
    _emit(){
      ...
      this._count++;
      const sample={ t_ms:now(), accel:a.slice(), gyro:(this._last.gyro||[0,0,0]).slice(),
                     mag:this._last.mag?this._last.mag.slice():null };
    ...
    stop(){
      ...
      else if(this._dm) global.removeEventListener('devicemotion',this._dm,true);
      this._cb=null;
    }
  }
